...
@onready var health_bar = $"HUD Layer/HUD/health bar"
...
func _ready():
	...
	load_tracks()
	if not Save.get_data("gameplay","downscroll",false):
		health_bar.global_position.y = 720 - health_bar.global_position.y
		for i:Node2D in strums.get_children():
			i.position.y = 720 - i.position.y
...
func _process(delta):
	for i:AudioStreamPlayer in tracks.get_children():
		var _t:float = i.get_playback_position() + AudioServer.get_time_since_last_mix()
		Conductor.time = _t
...
func spawn_notes():
	for i in chart.notes.size():
		...
		if  chart.notes[i].hit_time - Conductor.time >= 1.5: break
...
func player_note_hit(note:Note):	note.queue_free()
